...
def train_fold(fold_idx, train_indices, val_indices, full_dataset, embedding_matrix,
               vocab, device, num_folds):
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        print(f"  Epoch {epoch+1}: train_f1={train_metrics['f1']:.4f} "
...
def main():
    ...
    for fold_idx, (train_indices, val_indices) in enumerate(skf.split(np.zeros(len(labels)), labels)):
        ...
        best_state, best_f1 = train_fold(
            ...
            full_train_dataset, embedding_matrix, vocab, device, num_folds
        )
    ...
    top_models = [fold_models[i] for i in top_indices]

    # Ensemble predictions on test set
    ensemble_preds = ensemble_predict(top_models, test_loader, device)
    test_labels = np.array([item['label'] for item in test_data])
    ensemble_metrics = calculate_metrics(ensemble_preds, test_labels)

    print(f"\nTest Set (ensemble of top {top_k} folds):")
    ...
    for i, model in enumerate(fold_models):
        ...
        metrics = calculate_metrics(preds, test_labels)
        marker = "*" if i in top_indices else " "
        print(f"  {marker} Fold {i+1}: acc={metrics['accuracy']:.4f} f1={metrics['f1']:.4f}")
